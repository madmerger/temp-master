...
The frontend is built with Vite, React 18, and TypeScript. Copy `.env.example` to `.env` to
configure `VITE_API_URL`; it defaults to `https://snakeroom.fly.dev`, while an empty value
uses same-origin `/api` (the Vite development proxy targets `localhost:8000`).
...
5. Build or typecheck the frontend when needed:
   ```bash
   npm run build
   npm run typecheck
   ```
...
Docker builds the frontend and FastAPI serves the generated `dist/` files.
