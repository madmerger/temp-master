...
from app.models import MeterDevice, MeterReading, data_store
    ...
    async def test_collect_data_updates_datastore(self, reset_data_store, temp_db_path):
        ...
        try:
            ...
            with patch.object(config, "SWITCHBOT_TOKEN", "test-token"), \
                 ...
                 patch("app.services.switchbot.fetch_devices", new_callable=AsyncMock) as mock_fetch_devices, \
                 patch("app.services.switchbot.fetch_device_status", new_callable=AsyncMock) as mock_fetch_status:
    ...
    async def test_collect_data_rate_limit_breaks_loop(self, reset_data_store, temp_db_path):
        ...
        try:
            ...
            with patch.object(config, "SWITCHBOT_TOKEN", "test-token"), \
                 patch.object(config, "SWITCHBOT_SECRET", "test-secret"), \
                 patch("app.services.switchbot.fetch_devices", new_callable=AsyncMock) as mock_fetch_devices, \
                 patch("app.services.switchbot.fetch_device_status", side_effect=mock_fetch_status):
