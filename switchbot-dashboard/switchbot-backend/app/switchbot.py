...
from app.models import DataStore, MeterDevice
...
async def call_switchbot_api(endpoint: str, data_store: DataStore, device_id: Optional[str] = None) -> dict:
...
async def fetch_devices(data_store: DataStore) -> list[MeterDevice]:
    response = await call_switchbot_api("/devices", data_store)
...
async def fetch_device_status(device_id: str, data_store: DataStore) -> dict:
    response = await call_switchbot_api(f"/devices/{device_id}/status", data_store, device_id=device_id)
