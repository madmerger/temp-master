...
export function MeterCard({ meter, history, timeScale, darkMode, stale }: MeterCardProps) {
  return (
    <article className={`overflow-hidden rounded-2xl border shadow-lg ${stale ? 'border-amber-300 bg-amber-50 dark:border-amber-700/70 dark:bg-amber-950/30' : 'border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800'}`}>
      ...
      <div className="px-5 py-4">
        ...
        {stale ? <p className="py-8 text-sm text-amber-800 dark:text-amber-200">履歴データの取得対象外</p> : <TemperatureChart history={history} timeScale={timeScale} darkMode={darkMode} />}
